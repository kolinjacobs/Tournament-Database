...
def registerPlayer(name):
    ...
    c.execute("INSERT INTO players (id, player_name,matches)"
              "VALUES (DEFAULT, %s, 0);",
              (name,))
    DB.commit()
    c.close()
...
def playerStandings():
    ...
    c.execute("SELECT * FROM wincounter ORDER BY wins DESC")
    players = []
...
def reportMatch(winner, loser):
    ...
    for row in c.fetchall():
        player = (int(row[0]), str(row[1]), int(row[2]))
        players.append(player)
        if int(row[0]) == winner:
    ...
    c.execute("INSERT INTO matches (id,winner,loser)"
              "VALUES (DEFAULT, %s, %s);",
              (winner, loser,))
    # adds a match to the player table
